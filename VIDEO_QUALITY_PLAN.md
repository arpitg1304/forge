...
| 4 ✅ | **tier-1 motion** | `motion.py`: Farnebäck flow magnitude (`--min-motion`/`no_motion`), pixel-space LDLJ smoothness (`shaky`), global-affine cam/scene split, cut detection (histogram drop **and** flow spike → `cut_detected`); `--video-level motion`, wired into `forge filter`. Blank-frame guard; opencv added to the `[video]` extra (lazy import). | opencv |
| 5 ✅ | **`--workers`** | `forge quality --workers N`: parallelize episode analysis across processes (`spawn` ProcessPoolExecutor, contiguous chunks) in `forge/quality/parallel.py`. Identical results to sequential; speedup scales with per-episode decode cost (Tier 1/2), proprio stays sequential-fast. | none |
